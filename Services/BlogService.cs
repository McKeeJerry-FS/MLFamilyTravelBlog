...
namespace MLFamilyTravelBlog.Services
{
        ...
        public class BlogService : IBlogService
        {
            ...
            public async Task<List<BlogPost>> FilterBlogPostByCategory(int? categoryId)
            {
                try
                {
                    ...
                    category = await _context.Categories.Include(c => c.Posts)
                                                        .FirstOrDefaultAsync(c => c.Id == categoryId);
                    if (category != null)
                    {
                        blogPost = category.Posts!.ToList();
                    }
            ...
            public async Task<bool> IsValidSlugAsync(string? slug, int? blogPostId)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        BlogPost? blogPost = await _context.BlogPosts.AsNoTracking()
